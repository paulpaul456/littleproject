...
if(empty($sid)) {
    header('Location: farmer_product_read.php');
    exit;
}

$sql = "SELECT * FROM `farmer_product` WHERE `sid`=$sid";
...
//分類
$ttext_sql = "SELECT d.*, p.*FROM `farmer_product` d JOIN `product_class` p ON
d.class_sid=p.class_sid WHERE d.`sid`=$sid;";
$t_row = $pdo->query($ttext_sql)->fetch();


//證書
$text_sql = "SELECT d.*, p.*FROM `farmer_product` d JOIN `product_approve` p ON
d.approve_sid=p.sid WHERE d.`sid`=$sid;";
$p_row = $pdo->query($text_sql)->fetch();


//TAG的選項已選
// $tag_sql = "SELECT `tag_sid` FROM `farmer_product`WHERE `sid`=$sid;";
// $pp_row = $pdo->query($tag_sql)->fetch();
// $tag_stmt= json_decode($pp_row, JSON_UNESCAPED_UNICODE);
// print_r($pp_row);

$tag_sql = "SELECT `tag_sid` FROM `farmer_product`WHERE `sid`=$sid;";
$stmttt = $pdo->query($tag_sql);

//圖片陣列
while($r=$stmttt->fetch()){ 
$pic=json_decode($r['tag_sid'], JSON_UNESCAPED_UNICODE);
if(isset($pic)){                      
foreach($pic as $phot){
}
}
          ...
          <div class="col-md-12 col-lg-10">
            <div class="card card-user">
              <div class="card-header">
                <h5 class="card-title text-info">編輯商品</h5>
              </div>
                ...
                <input type="hidden" name="sid" value="<?= $row['sid'] ?>">
                  ...
                  <div class="col-md-4 pr-1" style="display: none">
                    ...
                    <div class="col-md-4 pr-1">
                      <div class="form-group">
                        <label>商品名稱</label>
                        <input type="text" class="form-control" placeholder="我是商品(限50字)" name="name" id="name" value="<?= htmlentities($row['name']) ?>">
                    ...
                    <div class="col-md-4 px-1">
                      <div class="form-group">
                        <label>價錢</label>
                        <input type="text" class="form-control" placeholder="價位售價" name="price" id="price" value="<?= htmlentities($row['price']) ?>">
                        <small id="priceHelp" class="form-text text-danger"></small>
                      </div>
                    </div>
                    <div class="col-md-4 pl-1">
                      <div class="form-group">
                        <label for="exampleInputEmail1">規格</label>
                        <input type="text" class="form-control" placeholder="EX:250g/袋(30字)" name="specification" id="specification" value="<?= htmlentities($row['specification']) ?>">
                  ...
                  <div class="row">
                    <div class="col-md-2 pr-1">
                      <div class="form-group">
                        ...
                        <select name="place" id="place" class="custom-select custom-select-m" >
                          <option><?= htmlentities($row['place']) ?></option>
                          <option>北部</option>
                          <option>中部</option>
                          <option>南部</option>
                          <option>東部</option>
                    ...
                    <div class="col-md-2 pr-1">
                      <div class="form-group">
                        ...
                        <select name="class_sid" id="class_sid" class="custom-select custom-select-m" >
                          <option value="<?= $t_row['class_sid']?>"><?= $t_row['name'] ?></option>
                    ...
                    <div class="col-md-2 pr-1">
                      <div class="form-group">
                        ...
                        <select name="color" id="color" class="custom-select" >
                          <option><?= htmlentities($row['color']) ?></option>
                          <option>黑</option>
                          <option>白</option>
                          <option>黃</option>
                          <option>綠</option>
                    ...
                    <div class="col-md-2 pr-1">
                      <div class="form-group">
                        ...
                        <select class="custom-select custom-select-m" name="approve_sid"  >
                            <option value="<?= $p_row['sid']?>"><?=  $p_row['name']?></option>
                    ...
                    <div class="col-md-4 pl-1">
                      <div class="form-group py-1">
                        <label>庫存</label>
                        <input type="number" class="form-control" placeholder="商品庫存量" name="stock" id="stock" value="<?= htmlentities($row['stock']) ?>">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label>小標</label>
                        <input type="text" class="form-control" placeholder="芋頭園芋頭香(100字)" name="subtitle" value="<?= htmlentities($row['subtitle']) ?>">
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label>成分</label>
                        <input type="text" class="form-control" placeholder="白色的粉10g 藍色的粉20g(250字)" name="content" id="content" value="<?= htmlentities($row['content']) ?>">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label>內文</label>
                        <textarea class="form-control textarea"  placeholder="" name="writing" id="writing" ><?= htmlentities($row['writing']) ?></textarea>
                      ...
                      <div class="col-md-12">
                        ...
                        <div class="my-4">
                          <?php foreach($tRows as $tr): ?>
                           <input class="mx-1" name="tag[]" type="checkbox" value="<?= $tr['sid']?>" 
                           <?php if(isset($pic)){                      
                                 foreach($pic as $phot){  ?>
                           <?=  $phot==$tr['sid'] ? 'checked' : '' ?> 
                                 <?php  } } ?>
                  ...
                  <div class="row">
                      <div class="col-md-12">
                          ...
                          <div id="list1" class="border border-dark d-flex " placeholder="" value="<?= htmlentities($row['picture']) ?>" >
                            <?php    $pic=json_decode($row['picture'], JSON_UNESCAPED_UNICODE);
                             if(isset($pic)){
                         foreach($pic as $phot){
...
const required_fields = [
    ...
    function checkform(){ 
      ...
      let fd = new FormData(document.product_form);
        ...
        if(isPass){
               fetch('farmer_product_edit_api.php', {
                method: 'POST',
                body: fd,
            })
              .then(response=>{
